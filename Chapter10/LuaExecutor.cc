...
namespace
{
    ...
    {
        (void)ud;
        std::cout << "[luaAlloc] ptr=" << std::hex << ptr << std::dec
                  << ", osize=" << osize << ", nsize=" << nsize;
        void *newPtr = NULL;
        if (nsize == 0)
        {
            free(ptr);
...
LuaExecutor::LuaExecutor(const LuaExecutorListener &listener, bool overrideAllocator)
    : L(overrideAllocator ? lua_newstate(luaAlloc, NULL) : luaL_newstate()), listener(listener)
{
    luaL_openlibs(L);
